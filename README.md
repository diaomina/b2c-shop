...
​		在每列操作栏内通过点击[编辑]按钮可以修改广告的信息，点击按钮后将会弹出编辑广告的弹窗界面，界面表单将会自动填充原始数据，管理员修改数据后点击[提交]按钮更新至数据库，点击[重置]按钮将会恢复表单原始数据。更新成功或者失败将会在界面上弹出对应的消息提醒。

​	（5）广告删除

​		在每列操作栏内通过点击[删除]按钮可以删除这条广告。被删除的广告将从广告列表消失，且不可以恢复。
